...
간단한 예제를 하나 작성해보자. 앞서 포스팅했던 [스프링 프레임워크 개요](https://yuunjuu.github.io/spring/SpringFramework/#%EB%8B%A4%ED%98%95%EC%84%B1%EC%9D%84-%EC%9D%B4%EC%9A%A9%ED%95%98%EC%97%AC-%EA%B2%B0%ED%95%A9%EB%8F%84-%EB%82%AE%EC%B6%94%EA%B8%B0)의 예제를 참조한다.
...
여기서 중요한 점은 TV를 교체할 때, 스프링 설정 파일 applicationContext.xml 파일만 수정하면된다는 점이다.
...
2. scope
    ...
    스프링 컨테이너는 컨테이너가 생성한 bean을 어느 범위에서 사용할 수 있는지를 지정할 수 있는데 이 때 scope 속성을 사용한다. 이 scope 속성값의 기본값은 싱글톤으로, 해당 bean이 스프링 컨테이너에 의해 하나만 생성되어 운용되도록 한다. singleton이 아닌 prototype르오 지정할 경우에는 스프링 컨테이너는 해당 <bean>이 요청될 때마다 매번 새로운 객체를 생성하여 반환한다.
